...
public class Rover {
	...
	private static String state = "0,0,N";
	...
	public String executeCommand(String commands) {
		...
		for(String command : commandArray) {
			String[] currentState = state.split(state);
			
			String direction = Helpers.getNewDirection(currentState[DIRECTION_INDEX], command);
			String cordinates = Helpers.getNewCordinates(Integer.valueOf(currentState[X_INDEX]), 
					Integer.valueOf(currentState[Y_INDEX]), currentState[DIRECTION_INDEX], command);
